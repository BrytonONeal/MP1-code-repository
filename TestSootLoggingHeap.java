		...
		if(!b.getMethod().getDeclaringClass().getName().equals("Log"))
		{
		    ...
		    while(it.hasNext()){
		    	...
		    	if (stmt.containsFieldRef()) {
		    		...
		    		AssignStmt value = (AssignStmt)stmt;
		    		try {
		    			boolean write = true;
		    			boolean stat = false;
						if (stmt.getDefBoxes().get(0).getValue().getClass() == Class.forName("soot.jimple.internal.JimpleLocal")){
							write = false;
						}
						Value valuet;
						if(stmt.getFieldRef().getField().isStatic()) {
							stat = true;
							valuet = value.getRightOp();
						}
						else {
							valuet = value.getLeftOp();
						}
						Log.logFieldAcc(Thread.currentThread(),stmt.getFieldRef().toString(),stat,write,valuet);
